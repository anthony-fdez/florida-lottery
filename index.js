    ...
    $(element)
      ...
      .each((index, element) => {
        ...
        if (row !== "" && row.length === 47) {
          ...
          const number1 = splitedRow.slice(10, 15).join("").split("-").join("");

          const date2 = splitedRow.slice(16, 24).join("");
          const period2 = splitedRow[25];
          const number2 = splitedRow.slice(26, 31).join("").split("-").join("");

          const date3 = splitedRow.slice(32, 40).join("");
          const period3 = splitedRow[41];
          const number3 = splitedRow.slice(42, 47).join("").split("-").join("");
...
const saveToJSONFile = ({ data }) => {
  ...
  fs.writeFile("allDates.json", json, () => {
